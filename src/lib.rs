...
pub fn grav_accel_ned(lat: f64, height: f64) -> Vector3<f64> {
    ...
    let g_0 =
        9.7803253359 * (1.0 + 0.001931853 * sin_lat_squared) / sqrt(1.0 - e * e * sin_lat_squared);
    ...
    Vector3::new(
        ...
        g_0 * (1.0
            - (2.0 / radius_wgs84)
                * (1.0
                    + f * (1.0 - 2.0 * sin_lat_squared)
                    + (pow(omega_ie, 2.0) * pow(radius_wgs84, 2.0) * b / mu))
                * height
            + (3.0 * pow(height, 2.0) / pow(radius_wgs84, 2.0))), // Calculate down gravity using (2.139)
    )
}
...
pub fn comp_lat_lon_height(pos_ecef: &Vector3<f64>) -> (f64, f64, f64) {
    let e = 0.0818191908425; // WGS84 eccentricity
    let a = WGS84.a;
    let lon = libm::atan2(pos_ecef[1], pos_ecef[0]);

    let k1 = sqrt(1.0 - e * e) * libm::fabs(pos_ecef[2]);
    let k2 = e * e * a;
    ...
    let D = P * P * P + Q * Q;
    let V = pow(sqrt(D) - Q, 1.0 / 3.0) - pow(sqrt(D) + Q, 1.0 / 3.0);
    let G = 0.5 * (sqrt(E * E + V) + E);
    let T = sqrt(G * G + (F - V * G) / (2.0 * G - E)) - G;

    let lat = libm::copysign(1.0, pos_ecef[2])
        * libm::atan((1.0 - T * T) / (2.0 * T * sqrt(1.0 - e * e)));

    let height = (beta - a * T) * libm::cos(lat)
        + (pos_ecef[2] - libm::copysign(1.0, pos_ecef[2]) * a * sqrt(1.0 - e * e)) * sin(lat);
