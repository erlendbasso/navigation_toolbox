...
pub fn lat_lon_height_to_ecef(lat: f64, lon: f64, height: f64) -> Vector3<f64> {
    ...
    let radius_n = a / f64::sqrt( 1.0 - f64::powi(e, 2) * f64::powi(f64::sin(lat), 2) );
    Vector3::new(
        (radius_n + height) * f64::cos(lat) * f64::cos(lon),
        (radius_n + height) * f64::cos(lat) * f64::sin(lon),
        (radius_n * ( 1.0 - f64::powi(e, 2) ) + height) * f64::sin(lat)
    )
}
...
pub fn lat_height_to_grav_accel_ned(lat: f64, height: f64) -> Vector3<f64> {
    ...
    let sin_lat_squared = f64::sin(lat) * f64::sin(lat);
    let g_0 = 9.7803253359 * (1.0 + 0.001931853 * sin_lat_squared) / f64::sqrt(1.0 - e * e * sin_lat_squared);

    Vector3::new(
        -8.08e-9 * height * f64::sin(2.0 * lat), // Calculate north gravity using (2.140)
        0.0, // East gravity is zero
        g_0 * (1.0 - (2.0 / radius_wgs84) * (1.0 + f * (1.0 - 2.0 * sin_lat_squared) +
        (f64::powi(omega_ie, 2) * f64::powi(radius_wgs84, 2) * b / mu)) * height + (3.0 * f64::powi(height, 2) / f64::powi(radius_wgs84, 2)))   // Calculate down gravity using (2.139)
    )
    }
